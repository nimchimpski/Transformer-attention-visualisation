...
def main():
    ...
    ## print(f"---tokenize...tokn_id= {tokenizer.mask_token_id}")
...
def get_mask_token_index(mask_token_id, inputs):
    ...
    if mask_token_id not in inputs['input_ids'][0]:
        return None
    tokenslist = list(inputs['input_ids'][0])
    index = tokenslist.index(mask_token_id)
    return index
...
def visualize_attentions(tokens, attentions):
    ...
    # print(f"+++attentions[0]]={attentions[0][0]}")
    numlayers = len(attentions)
    # print(f"+++numlayers={numlayers}")
    numheads = len(attentions[0][0])
    ...
    # one diagram per head, for every layer!
    # num layers?
    # num heads?
    # put these on diagram
